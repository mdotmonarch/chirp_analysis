...
clean_electrodes = Float64[]

for dataset in datasets
	println("Processing dataset: ", dataset)
	# if directory does not exist, create it
	if !isdir("./processed_data/"*dataset*"/")
		mkpath("./processed_data/"*dataset*"/")
	end

	# processing pipeline
	#select_with_signal_to_noise_ratio(dataset, 5)
	#normalize_signals(dataset)
	average_signal(dataset)
	...
	h5open("./processed_data/"*dataset*"/"*dataset*"_processed.h5", "r") do processed_file
		global clean_electrodes = [clean_electrodes; length(read(processed_file, "clean_electrodes/data"))]
	end

	for r in r_list
		compute_complexity_curve(dataset, "MSE", 2, r, [i for i in 1:80])
		#compute_complexity_curve(dataset, "CMSE", 2, r, [i for i in 1:80])
		compute_complexity_curve(dataset, "RCMSE", 2, r, [i for i in 1:80])
	end
end

println("")
println("....................................................")
println("Average number of clean electrodes: ", mean(clean_electrodes))